...
1. 点击底部的输入框
2. 输入您的消息
3. **💡 小技巧**：您可以将文件拖拽到输入框中，自动获取文件路径
4. 按Ctrl+Enter键或点击发送按钮
5. AI将回复您！
...
**包里有什么：**
- 📁 **文件操作**（`files.py`）：读取、写入、复制、移动、删除文件
- 👁️ **OCR**（`ocr.py`）：从图片和PDF中提取文本
...
**注意**：这需要同时安装[初学者工具包](#第5步ai命令神奇的部分)和运行 [Umi-OCR](https://github.com/hiroi-sora/Umi-OCR)。
...
> 💡 **需要更多详情？** 请参阅本文档末尾的[完整参数参考](#完整参数参考)。
...
1. 设置 → 导出配置
2. 选择保存位置（建议保存到桌面）
3. 文件名格式：`{chat_name}-{timestamp}.zip`
4. **包含内容：**
   - `settings.json` - 所有设置（为安全起见排除 API 密钥）
   - `tools/` - 所有 MCP 工具和配置
5. **排除内容：**
   - 聊天历史（`{chat_name}_ai.json`）
   - API 密钥（存储在 `.confignore` 中）
...
**方法1：拖拽导入（最简单）**
1. 找到您的 ZIP 文件（例如 `chat1-20250119123456.zip`）
2. 直接将其拖拽到左侧边栏的聊天列表上
3. 配置将自动导入为新对话
4. 如果存在同名聊天，将自动重命名（例如 `chat1_1`）
